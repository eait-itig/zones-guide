...
You can create a new Jupyter notebook zone on your personal account by simply
requesting the image named `jupyter` for a new zone:

[source,shell-session]
....
$ triton inst create --wait jupyter z1-standard
Creating instance xxxxxxxx (.., jupyter@20250225)
Created instance xxxxxxxx (..) in 1m1s <1>
....
...
If you're creating a Jupyter zone on a group account or one you want to use
to collaborate with others, you will need to set metadata keys to let the
image know which user "owns" the zone (the Jupyter notebooks will run as
this user) and any other UQ users who are allowed:

[source,shell-session]
....
$ triton inst create --wait \
  -m owner=uqxyz -m uq_users=s4xxxxx \
  jupyter z1-standard
Creating instance xxxxxxxx (.., jupyter@20250225)
Created instance xxxxxxxx (..) in 1m1s <1>
....
