...
While `docker` and `docker-compose` do not work within a zone (since zones are
not truly running a Linux kernel they don't have some of the features which
`docker` needs to operate), we can create zones using Docker images.

To do this we actually use the real `docker` command, but we run it on the
same machine we run the `triton` command (not in a zone). With some
configuration, the `docker` command will operate on the UQcloud Triton cluster
instead of a local Docker daemon.
...
As the instructions say at the bottom of the output, we then must run
...
in each container. If it's difficult to make your main application server
deal correctly with these constraints (e.g. because you want to deploy an
off-the-shelf image), you may find it simpler to use a docker container based
on the `webproject` image placed "in front" of your back-end container.

We can do this by giving the UUID of the latest `webproject` image as the
image name to `docker run` or `docker create`, with the entrypoint specified
as `/usr/local/sbin/docker-entrypoint`:
...
execute `nginx`. Note that other services will not be started like the would
with a regular `webproject` zone.
...
backend container, however, you can set the Docker label `proxy_to` to a HTTP
...
NOTE: If you do decide to use your authentication certificate and key in a CI
      context, make certain you do not ever commit them to your code
